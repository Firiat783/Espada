class ItemUser < ActiveRecord::Base
  ...
  # считает полную сумму
  # @params user [User] Объект пользователя, для которого считаем корзину
  # @return Float
  def self.total_price(user)
    #посчитать
    #созздаем переменную Сумма
    total = 0.0
  ...
  #Считаем стоимость текущей позиции в корзине - если товаров одного наиенования
  #несколько
  def full_price
    quantity * item.price
  end
